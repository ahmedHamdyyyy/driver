...
import 'package:taxi_driver/screens/ComplaintScreen.dart';
...
import 'settings/settings_screen/presentation/pages/chat_screen.dart';
...
class DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<BitmapDescriptor> getNetworkImageMarker(String? imageUrl) async {
    if (imageUrl == null || imageUrl.isEmpty) {
      print("لا يوجد رابط صورة، سيتم استخدام الأيقونة الافتراضية");
    ...
    try {
      ...
      if (uri.host.isEmpty) {
        throw FormatException('لا يوجد مضيف في الرابط', imageUrl);
      }
      ...
      if (bytes.isEmpty || bytes.length < 100) {
        throw Exception('بيانات الصورة صغيرة جداً أو فارغة');
      }

      // محاولة استخدام التحويل المباشر
      try {
        return BitmapDescriptor.fromBytes(bytes);
      } catch (e) {
        print("فشل التحويل المباشر: $e");

        // استخدام طريقة أخرى للتحويل
        ...
        if (byteData == null) {
          throw Exception('فشل تحويل بيانات الصورة');
        }
      ...
      print("خطأ في إنشاء علامة من URL: $e");
      print("تفاصيل الخطأ: $s");
  ...
  void init() async {
    ...
    // تعيين أيقونة السائق الافتراضية أولاً لضمان وجودها
    driverIcon = await BitmapDescriptor.fromAssetImage(
        ImageConfiguration(devicePixelRatio: 2.5),
        Platform.isIOS ? DriverIOSIcon : DriverIcon);

    // تحديث موقع السائق والخريطة عند استلام إحداثيات جديدة
    Geolocator.getPositionStream().listen((event) {
      driverLocation = LatLng(event.latitude, event.longitude);

      // حفظ الموقع في التخزين المحلي
      sharedPref.setDouble(LATITUDE, event.latitude);
      sharedPref.setDouble(LONGITUDE, event.longitude);

      // إضافة علامة السائق إلى الخريطة في كل مرة يتغير فيها الموقع
      if (driverIcon != null) {
        try {
          // حذف علامة السائق القديمة إن وجدت
    ...
    // ضمان إظهار أيقونة السائق عند بدء التطبيق
    updateDriverMarker();
  }

  // دالة لتحديث موقع السائق والأيقونة
  void updateDriverMarker() {
    if (driverLocation != null && driverIcon != null) {
      try {
        ...
        print('Error in updateDriverMarker: $e');
      }
    }
  }
  ...
  Future<void> setSourceAndDestinationIcons() async {
    try {
      // تحميل أيقونة السائق
      if (service_marker != null && service_marker.isNotEmpty) {
        try {
      ...
      // تحديث أيقونة السائق على الخريطة
      updateDriverMarker();

      // تحميل أيقونات المصدر والوجهة
      ...
      print("Error in setSourceAndDestinationIcons: $e");
    }
  }
  ...
  onMapCreated(GoogleMapController controller) {
    ...
    // تحديث علامة السائق عند إنشاء الخريطة
    updateDriverMarker();
  }
      ...
      if (value.onRideRequest != null) {
        ...
        if (value.estimated_price != null && value.estimated_price.isNotEmpty) {
          try {
            ...
            estimatedDistance = num.tryParse(
                value.estimated_price[0]['dropoff_distance_in_km'].toString());
          ...
          if (servicesListData!.status == COMPLETED &&
            ...
            if (appStore.isShowRiderReview == '0') {
              ...
              await ratingReview(request: req).then((value2) async {
                await rideDetail(rideId: value.onRideRequest!.id!)
                    .then((value3) {
                  RideService rideService = RideService();
                  rideService.updateStatusOfRide(
                      rideID: value.onRideRequest!.id!,
                      req: {'on_rider_stream_api_call': 0});
                  if (value3.payment != null &&
                      value3.payment!.paymentStatus == PENDING) {
  ...
  getNewRideReq(int? riderID, {bool? refresh}) async {
    ...
    await rideDetail(rideId: riderID).then((value) async {
      ...
      if (value.ride_has_bids != null) {
        ...
        try {
          admin_commission = num.tryParse(
                  value.estimated_price[0]['admin_commission'].toString()) ??
              0;
          surge_charge = num.tryParse(
                  value.estimated_price[0]['fixed_charge'].toString()) ??
              0;
          commission_type =
              value.estimated_price[0]['commission_type'].toString();
        } catch (e, s) {
        ...
        try {
          ...
          if (value.estimated_price != null &&
            ...
            try {
              estimatedTotalPrice = num.tryParse(
                  value.estimated_price[0]['total_amount'].toString());
              estimatedDistance = num.tryParse(value.estimated_price[0]
                      ['dropoff_distance_in_km']
                  .toString());
              if (estimatedDistance.toInt() == 0) {
                estimatedDistance = num.tryParse(
                    value.estimated_price[0]['distance'].toString());
              }
              distance_unit =
                  value.estimated_price[0]['distance_unit'].toString();
            } catch (e, s) {
  ...
  Future<void> setMapPins() async {
    try {
      if (servicesListData != null &&
          servicesListData!.multiDropLocation != null &&
          servicesListData!.multiDropLocation!.isNotEmpty) {
        // مسح العلامات القديمة ماعدا علامة السائق
        markers.removeWhere((marker) => marker.markerId.value != "driver");

        // التأكد من وجود علامة السائق
        updateDriverMarker();
  ...
  Future<void> startLocationTracking() async {
    await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high)
      ...
      driverLocation = LatLng(value.latitude, value.longitude);

      // حفظ موقع السائق في التخزين المحلي
          ...
          positionStream =
            ...
            if (d != null && DateTime.now().difference(d).inSeconds > 10) {
              if (appStore.isLoggedIn) {
                ...
                await updateStatus(req).then((value) {
                  // تحديث أيقونة السائق على الخريطة بعد تحديث الموقع
                  updateDriverMarker();
                  setState(() {});
                }).catchError((error) {
                  log(error);
                                                                      ...
                                                                      children: [
                                                                        Expanded(
                                                                          child:
                                                                            ...
                                                                            decoration:
                                                                                inputDecoration(context, label: language.amount),
                                                                            maxLines:
                                                                                1,
                                                                            minLines:
                                                                                1,
  ...
  // Complaint button at the bottom left of the screen
  Widget complaintButton() {
    return Positioned(
      ...
      child: GestureDetector(
        onTap: () {
          if (servicesListData != null && riderData != null) {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => ComplaintScreen(
                  driverRatting: DriverRatting(rating: 0.0),
                  complaintModel: null,
                  riderModel: RiderModel(
                    riderId: riderData!.id,
                    riderName:
                        riderData!.firstName! + " " + riderData!.lastName!,
                    riderProfileImage: riderData!.profileImage,
                    id: servicesListData!.id,
                  ),
                ),
              ),
            ...
            toast("لا توجد رحلة متاحة للشكوى");
          }
        },
        child: Container(
          height: 50,
