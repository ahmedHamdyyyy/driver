...
/**
 * ====================================================
 * NewSignUpWidget - نموذج تسجيل جديد 100% آمن
 * ====================================================
 * 
 * ✅ بدون أي استخدام لـ catchError
...
class _NewSignUpWidgetState extends State<NewSignUpWidget> {
  ...
  // معالج الأخطاء الآمن 100%
  String _getErrorMessage(dynamic error) {
    String errorStr = error.toString().toLowerCase();

    // أخطاء Firebase Auth المحددة
  ...
  Future<void> _signUp() async {
    ...
    try {
      ...
      final userCredential = await _createFirebaseUserSafely();
      ...
      if (userCredential?.user != null) {
        ...
        await _saveUserToFirestoreSafely(userCredential!.user!);
  ...
  // إنشاء مستخدم Firebase بطريقة آمنة
  Future<UserCredential?> _createFirebaseUserSafely() async {
    try {
  ...
  // حفظ البيانات في Firestore بطريقة آمنة
  Future<void> _saveUserToFirestoreSafely(User user) async {
    try {
  ...
  // التسجيل في API بطريقة آمنة بدون catchError
  Future<void> _registerWithAPISafely() async {
    try {
      ...
      // استخدام HTTP مباشرة بدلاً من signUpApi
      await _callAPIDirectly(req);
    } catch (e) {
      print('API Error: $e');
      throw e;
    }
  ...
  // استدعاء API مباشرة بدون catchError
  Future<void> _callAPIDirectly(Map<String, dynamic> request) async {
    try {
      final baseUrl =
          'https://mighty-taxi.appmeetmighty.com/api'; // ضع رابط API الصحيح هنا
      final url = Uri.parse('$baseUrl/driver-register');
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        if (data['data'] != null) {
          ...
          await sharedPref.setString(TOKEN, userData['api_token'] ?? '');
          await sharedPref.setString(USER_TYPE, userData['user_type'] ?? '');
          await sharedPref.setString(FIRST_NAME, userData['first_name'] ?? '');
          await sharedPref.setString(LAST_NAME, userData['last_name'] ?? '');
          await sharedPref.setString(
              CONTACT_NUMBER, userData['contact_number'] ?? '');
          await sharedPref.setString(USER_EMAIL, userData['email'] ?? '');
          await sharedPref.setString(USER_NAME, userData['username'] ?? '');
          await sharedPref.setInt(USER_ID, userData['id'] ?? 0);
          await sharedPref.setBool(IS_LOGGED_IN, true);
        }
      } else {
        final error = json.decode(response.body);
        throw error['message'] ?? 'خطأ في الخادم';
      }
    } catch (e) {
